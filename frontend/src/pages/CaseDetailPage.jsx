...
const CaseDetailPage = () => {
  ...
  const handleReportSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`/api/emergency-cases/${id}/reports`, {
        method: 'POST',
        headers: {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500">The requested emergency case could not be found.</p>
          <Link
            to="/emergency-cases"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="px-6 py-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex items-center text-sm text-gray-600">
                    ...
                    <span>{caseData.location?.address}, {caseData.location?.district}, {caseData.location?.state}</span>
                  </div>
                  <div className="flex items-center text-sm text-gray-600">
            ...
            <div className="bg-white shadow rounded-lg">
              <div className="px-6 py-4 border-b border-gray-200">
                <h2 className="text-lg font-medium text-gray-900">Affected People</h2>
              </div>
              <div className="px-6 py-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-red-600">{caseData.affectedPeople?.injured || 0}</div>
                    <div className="text-sm text-gray-600">Injured</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold text-gray-600">{caseData.affectedPeople?.deceased || 0}</div>
                    <div className="text-sm text-gray-600">Deceased</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold text-blue-600">{caseData.affectedPeople?.evacuated || 0}</div>
            ...
            {/* Assigned Agencies */}
            {caseData.assignedAgencies && caseData.assignedAgencies.length > 0 && (
              <div className="bg-white shadow rounded-lg">
                <div className="px-6 py-4 border-b border-gray-200">
                  <h2 className="text-lg font-medium text-gray-900">Assigned Agencies</h2>
                </div>
                <div className="px-6 py-4">
                  <div className="space-y-3">
                    {caseData.assignedAgencies.map((agency, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div>
                          <div className="font-medium text-gray-900">{agency.agency}</div>
                          {agency.contactPerson && (
                            <div className="text-sm text-gray-600">Contact: {agency.contactPerson}</div>
                          )}
                          {agency.phone && (
                            <div className="text-sm text-gray-600">Phone: {agency.phone}</div>
                          )}
                        </div>
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          agency.status === 'Completed' ? 'bg-green-100 text-green-800' :
                          agency.status === 'Responding' ? 'bg-blue-100 text-blue-800' :
                          agency.status === 'Assigned' ? 'bg-yellow-100 text-yellow-800' :
                      ...
                      <div key={index} className="border border-gray-200 rounded-lg p-4">
                        ...
                        {report.adminResponse && (
                          <div className="mt-2 p-3 bg-gray-50 rounded">
                            <div className="text-sm font-medium text-gray-900 mb-1">Admin Response:</div>
                            <div className="text-sm text-gray-700">{report.adminResponse}</div>
                            {report.respondedAt && (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white shadow rounded-lg">
              <div className="px-6 py-4 border-b border-gray-200">
                <h2 className="text-lg font-medium text-gray-900">Submit Report</h2>
              </div>
              <div className="px-6 py-4">
              ...
              <div className="px-6 py-4 space-y-3">
                ...
                {caseData.verificationStatus?.verifiedAt && (
                  <div className="flex justify-between">
                    ...
                    <span className="text-sm font-medium text-gray-900">
                      {new Date(caseData.verificationStatus.verifiedAt).toLocaleDateString()}
                    </span>
                  </div>
                )}
